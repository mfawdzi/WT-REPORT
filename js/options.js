...
/**
 * 26100 -> "26+100". Inverse of kpMetres, for auto-filling the next KP.
 *
 * Clamped to what the "XX+XXX" field can hold (0 to 99+999) rather than
 * overflowing into a sixth digit -- a KP that no longer fits the shape would
 * print wrong everywhere it is read.
 */
WT.kpFromMetres = function (metres) {
  if (metres == null || isNaN(metres)) return '';
  var clamped = Math.max(0, Math.min(99999, Math.round(metres)));
  var km = Math.floor(clamped / 1000);
  var m = clamped % 1000;
  var kmText = km < 10 ? '0' + km : String(km);
  var mText = m < 10 ? '00' + m : m < 100 ? '0' + m : String(m);
  return kmText + '+' + mText;
};
