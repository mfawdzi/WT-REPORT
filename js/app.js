...
(function (WT) {
  ...
  /**
   * Custom in-page camera session. Opens once and stays open across all three
   * shots -- the operator taps the shutter for each photo instead of the
   * native camera app closing and reopening every time. Falls back to the
   * native picker if getUserMedia is missing, denied, or fails.
   */
  function openBurstCamera() {
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      // Denied, no camera, insecure context, etc. -- the native picker still works.
      $('camera-input').click();
    });
  }
  ...
  function runCameraSession(stream, shotsWanted) {
    ...
    function onDone() {
      stop();
      if (blobs.length) addPhotos(blobs, 'camera');
    }
  ...
  /** Opens the full-size stored photograph over everything else. */
  var previewUrl = null;
  function openPhotoPreview(blob) {
  ...
  $('photo-preview').addEventListener('click', function (event) {
    // Tapping the dark backdrop closes it too; tapping the photo itself should not.
  ...
  function renderPhotoStrip() {
    ...
    state.photos.forEach(function (photo, index) {
      ...
      image.src = url;
      image.alt = '';
      // The stored photograph, not the small thumbnail -- what the operator
      // taps to check is the same file that will go into the report.
      image.addEventListener('click', function () { openPhotoPreview(photo.blob); });
  ...
  function resetAfterSave() {
    state.form.kp = nextKpGuess(state.form.kp);
    state.form.other = '';
    state.form.condition = WT.OPTIONS.conditions[0].label;
    state.photos = [];

    $('f-kp').value = state.form.kp;
    $('f-other').value = '';
    syncCondition();
    renderPhotoStrip();
    updateReady();
   ...
   *
   * The caption travels WITH the photographs whenever the browser accepts both,
   * exactly as ROWPowerline does it — one tap, nothing to paste. That is the
   * output the brief asks for and it must stay the default.
   *
   * Only where the browser refuses the pair do the photographs go alone and the
   * caption have to be pasted, and the send screen then says so in steps. The
   * paste is a fallback, never the normal route.
   */
  ...
  function renderSend() {
    ...
    /* Steps only when there is something to do by hand. In the normal case the
       caption goes with the photographs and there is nothing to explain; showing
       a paste instruction there would teach the crew to do work the app has
       already done. */
    var pasting = effectiveMode() === 'files';
    ...
    $('send-note').textContent =
      sending.mode === 'none'
        ? 'Browser ini tidak bisa membagikan foto. Buka lewat Chrome (Android) atau Safari (iPhone).'
      : pasting ? 'Browser ini hanya mengirim foto — caption sudah disalin, tinggal ditempel.'
      : 'Foto dan caption dikirim bersamaan.';

    /* WHICH SHARE TARGET TO TAP. This is the whole difference between one tidy
       report and three messy ones, and it is not something an operator could
       ever work out from the result.

       Android's share sheet shows two rows. The top row is WhatsApp's
       direct-share shortcuts — one per recent chat, with the contact's photo.
       Tapping one of those hands the share straight to that chat, and WhatsApp
       captions each photograph on its own: three messages, the same report under
       each. Tapping the WhatsApp ICON in the row below opens WhatsApp's own chat
       picker and then its media editor, where a single caption covers the whole
       album: one message, one report.

       Same photographs, same caption, same call — two different results
       depending on which icon a thumb lands on. So it is stated on screen every
       time rather than left to memory. */
    $('send-tip').textContent = sending.mode === 'none' ? '' :
      'Di menu berbagi, pilih ikon WhatsApp di baris bawah — bukan foto kontak ' +
      'di baris atas. Baru pilih grupnya. Kalau lewat baris atas, caption akan ' +
      'terulang di setiap foto.';

    $('send-status').textContent = '';
    $('send-env').classList.add('hidden');
  }

  /** Which share this phone will actually make. */
  function effectiveMode() {
    var sending = state.sending;
    return sending ? sending.mode : 'none';
  }
  ...
  $('send-share').addEventListener('click', function () {
    ...
    /* Files and caption together when the browser accepts both — that is the
       output the brief asks for: one message, photos and report, nothing to
       paste. Where it does not, the photos go alone and the caption has to be
       pasted, which is why Salin Caption is always on screen rather than hidden
       behind a failure.

       In that fallback the clipboard is loaded here, inside the tap, and
       deliberately NOT awaited: waiting would spend the gesture that authorises
       the share sheet and the share would then be refused with NotAllowedError,
       which looks like every other share failure. */
    var payload = effectiveMode() === 'text'
      ? { files: sending.files, text: sending.caption }
      : { files: sending.files };
    if (effectiveMode() === 'files') copyCaption(sending.caption);
